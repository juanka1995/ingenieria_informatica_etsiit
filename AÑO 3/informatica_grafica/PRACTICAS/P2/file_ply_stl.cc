...
void _file_ply::read(vector<_vertex3f> &Vertices,vector<_vertex3i> &Triangles, bool &goodReaded)
{
  string Token;
  ...
  if(Token=="ply"){
    ...
    if(Token=="format"){
      ...
      if(Token=="ascii"){
        ...
        if(Token=="vertex"){
          get_token(Line_stream,Token);
          numVertex = stoi(Token); // Asignamos número de vertices
          ...
          if(Token=="face"){
            get_token(Line_stream,Token);
            numFaces = stoi(Token); // Asignamos número de caras
...
int _file_ply::getNumVertex() const {
  return numVertex;
}

int _file_ply::getNumFaces() const{
  return numFaces;
}