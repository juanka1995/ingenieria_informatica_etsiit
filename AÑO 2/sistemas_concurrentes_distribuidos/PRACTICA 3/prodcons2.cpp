...
#define Etiqueta_Productor    1
#define Buffer       5
#define Etiqueta_Consumidor   2
#define ITERS       20
#define TAM          5
...
void productor()
{
   ...
   for ( unsigned int i=0; i < 4 ; i++ )
   {
      ...
      // enviar 'value'
      MPI_Ssend( &value, 1, MPI_INT, Buffer, Etiqueta_Productor, MPI_COMM_WORLD );
   }
}
// ---------------------------------------------------------------------
...
void buffer()
{
   ...
   for( unsigned int i=0 ; i < ITERS*2 ; i++ )
   {
      ...
      {
         case 0:
            MPI_Recv( &value[pos], 1, MPI_INT, MPI_ANY_SOURCE, Etiqueta_Productor, MPI_COMM_WORLD, &status );
            cout << "Buffer recibe " << value[pos] << " de Productor " << status.MPI_SOURCE << endl << flush;
            pos++;
            break;
         case 1:
            MPI_Recv( &peticion, 1, MPI_INT, MPI_ANY_SOURCE, Etiqueta_Consumidor, MPI_COMM_WORLD, &status );
...
void consumidor()
{
   ...
   {
      MPI_Ssend( &peticion, 1, MPI_INT, Buffer, Etiqueta_Consumidor, MPI_COMM_WORLD );
